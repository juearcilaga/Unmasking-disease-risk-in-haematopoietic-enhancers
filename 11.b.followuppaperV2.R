...
library(plyranges)
# gene_Anotations<-readRDS("extData/gr_gene_Anotations38.rds")
gene_Anotations<-readRDS("gr_gene_Anotations38_lcincluded.rds")
...
SNPS.GENES.DF<-SNPS.GENES.DF[grepl("\\:", SNPS.GENES.DF$gene_ID)==FALSE,]%>% group_by(gene_name) %>% 
  filter(gene_size==max(gene_size))

write.table(SNPS.GENES.DF,"../OUTPUT/Lipid.GENESandlc.DF.txt", sep="\t")
