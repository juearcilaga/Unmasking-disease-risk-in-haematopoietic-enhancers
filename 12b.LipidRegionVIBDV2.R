...
dev.off()
ggtexttable(GENES.DF[, c(1, 6:10)], rows = NULL)
...
SNPsToEnh.summary[SNPsToEnh.summary$startSNPreg==start,]
#-----------------------------------------------------------------------------------------
# Chr 2 Neutrophils
# Chr 7 Eryth y ends
# chr9  chr7  chr6  chr5  chr4  chr21 chr2 
#-----------------------------------------------------------------------------------------
SetofEnh <- 12# ID: 4 (chr 6), 2 (chr 7), chr: 2 (ID: 12,13),21,4,2,9 only with gene
chromosome <-
  as.character(SNPsToEnh.summary.bychr[SNPsToEnh.summary.bychr$ID == SetofEnh, "chr"])
...
vertical.linesDF<-SNPsToEnh.summary %>% 
  select(SNPS,CHR_POS) %>% 
  left_join(SNPsToEnh.summary.bychr) %>%
  filter(ID==SetofEnh)

vertical.lines <-
  vertical.linesDF$CHR_POS

names(vertical.lines) <-
  vertical.linesDF$SNPS
...
ifelse(
  length(InRegionGenes$start) > 0,
  ifelse(
    InRegionGenes$start - min(mydata$start) < 20000,
