...
lipid.GENES.DF<-read.table("OUTPUT/Lipid.GENESandlc.DF.txt", sep="\t")
...
# for (SetofEnh in c(1:5, 7)) {
  ...
  SNPsToEnhM0123.summary$TRAITAbb <-
    c(
      "LDL",
      "HDL_TG_metT",
      "TG",
  ...
  # SNPsToEnhM0123.summary<-SNPsToEnhM0123.summary[SNPsToEnhM0123.summary$chr==chromosome,] %>%
  #   unite(labely, SNPS,TRAITAbb, sep=".")
  
  
  vertical.lines <-
    SNPsToEnhM0123.summary[SNPsToEnhM0123.summary$chr == chromosome, "CHR_POS"]
  
  # names(vertical.lines)<-
  #   SNPsToEnhM0123.summary[SNPsToEnhM0123.summary$chr==chromosome,"labely"]
  
  names(vertical.lines) <-
    SNPsToEnhM0123.summary[SNPsToEnhM0123.summary$chr == chromosome, "SNPS"]
  ...
  M012 <-
    unique(metadata[grepl("macrophage", metadata$CELL_TYPE), "SAMPLE_NAME"])
  ...
  Tabla <-
    SNPsToEnhM0123.summary[SNPsToEnhM0123.summary$chr == chromosome, ] %>% select(SNPS, DISEASE.TRAIT)
...
# }
# #
# by_sample+annotation_custom(ggplotGrob(Table.plot),
#                   xmin = 30000000, ymin = 6,
#                   xmax = 16)
...
lipid.GENES.DF$seqnames <- factor(
  lipid.GENES.DF$seqnames,
  levels = c("chr2", "chr6", "chr8", "chr11", "chr19", "chr22", "chrX")
)

lipid.GENES.DF<-with(lipid.GENES.DF, lipid.GENES.DF[order(seqnames),])

ggtexttable(lipid.GENES.DF[, c(1,6:11)], rows = NULL)


SNPsToEnhM0123.summary$chr <- factor(
  SNPsToEnhM0123.summary$chr,
  levels = c("chr2", "chr6", "chr8", "chr11", "chr19", "chr22", "chrX")
)
SNPsToEnhM0123.summary<-with(SNPsToEnhM0123.summary, SNPsToEnhM0123.summary[order(chr),])

SNPsToEnhM0123.summary$Enhancer_Size_kb<-SNPsToEnhM0123.summary$NEnh*200/1000
ggtexttable(SNPsToEnhM0123.summary[, c(1,3,5,12,2)], rows = NULL)
