...
#------------------------------------------------------------------------------
gr_EnhSNP[ grepl(trait, gr_EnhSNP$DISEASE.TRAIT) &
...
length(unique(EnhSNP.trait.celltypes$SNPS))#18
...
#------------------------------------------------------------------------------
GWAS.ENRICH.TRAIT <-GWAS.ENRICH.RESULTS[grepl(trait, GWAS.ENRICH.RESULTS$TRAIT),]
...
data.frame(chr = unique(EnhSNP.enriched$CHR_ID) )# 8,11,15,19
...
table(EnhSNP.enriched$CHR_ID) %>% data.frame()
#    CHR  Nenh
# 1   11   12
# 2   15    9
# 3   19   10
...
table(EnhSNP.enriched.summary$CHR_ID) %>% data.frame()
#    CHR  NSNPs
# 1   11    1
# 2   15    3
# 3   19    1
...
# Chromosomes 11: 12 enhancers 1 SNP window rs10838681
# Chromosomes 19: 2 enhancer 1 snp rs157582
# All in introns
...
## 300000 pares de bases alejado hacia la izquierda del inicio del fragmento
