...
#--------------------------------------------------------------------------------
#                     CONSERVED ENHANCERS BY CELLTYPE
#--------------------------------------------------------------------------------
...
#--------------------------------------------------------------------------------
#                     PLOT N ENHANCERS BY CELLTYPE
#--------------------------------------------------------------------------------

setwd("/mnt/nocode/juliana/PID/ThemeII")

StackObj.long <-
  read.delim("OUTPUT/Enhconservbund_all.txt", sep = "\t")

data <-StackObj.long[StackObj.long$NameState%in%c("AconE", "AeloE"),]

data[data$NameState=="AeloE","NameState"]<-"AconE"
data$PATTERN_NAME<-as.character(data$PATTERN_NAME)
data[data$CELL_TYPE2=="cytotoxic CD56 dim natural killer cell","PATTERN_NAME"]<-"NK"

# data$CELL_TYPE_Abb<-factor(data$CELL_TYPE_Abb, levels = unique(data$CELL_TYPE_Abb))

data2<-aggregate(data$NBins, by=list(CELL_TYPE2=data$CELL_TYPE2), FUN=sum)
data2<-data2[order(data2$x,decreasing = TRUE),]

data4<-left_join(data2, unique(data[, c("CELL_TYPE2","PATTERN_NAME", "CELL_TYPE_Abb")]))
data4$CELL_TYPE_Abb<-as.character(data4$CELL_TYPE_Abb)
data4[15,"CELL_TYPE_Abb"]<-"Natural Killer"


# data4_ordered<-data4[order(data4$x, decreasing = TRUE),]
data4$PATTERN_NAME <- 
   factor(data4$PATTERN_NAME,
   levels = unique(data4$PATTERN_NAME))
...
ggplot(data4, aes(y= Conserved_Enhancers, x=PATTERN_NAME)) + 
