...
mycelltypes <-
  metadata %>% count(., CELL_TYPE) %>% filter(., n > 1) %>% data.frame() %>% dplyr::select(CELL_TYPE)

mysamples <-
  metadata %>% filter(., CELL_TYPE %in% mycelltypes$CELL_TYPE) %>% dplyr::select(CELL_TYPE, SAMPLE_NAME)

Datavenous <- All_data[, colnames(All_data) %in% mysamples$SAMPLE_NAME]
dim(Datavenous)#[1] 15441337 #103
...
# Sankey_plot <- function(A_B, label_states, transition_name) {
  ...
  print(Changes_A_B[order(Changes_A_B$value, decreasing = TRUE),][1:10,])
  ...
  my_color <-
    'd3.scaleOrdinal(["#D3D3D3","#1B9E77","#d95f02","#A6761D","#E6AB02","#66A61E",
  ...
  # With networkD3, connection must be provided using id, not using real name like in 
  #the links dataframe.. So we need to reformat it.
  ...
  linkis$link.colr <-
    factor(linkis$link.colr,
           levels = c("othercol", "B_PoisEcol","B_AconEcol"))
...
# celltypes$MAIN_CELL_TYPE[c(14,17:19,22:23,25,27)]<-c("NK","monocyte", "T", "endothelial", 
#                           "megakaryocyte", "T", "B", "T")

# celltypes$CELL_TYPE_Abb<-as.character(celltypes$CELL_TYPE_Abb)
# celltypes$CELL_TYPE_Abb[c(14,17:19,22:23,25,27)]<-c("NK","monocyte", "effect memory CD8 T", "endothelial UmV prolif", 
#                           "megakaryocyte", "CD8 T", "CD38neg naive B", "CD4 T")
...
for (i in 1:nrow(chrlabels)) {
  ...
  my_list[[Chromstate]] <- colSums(ConservedDF == SymbState)
}
StackObj <- do.call(cbind, my_list)
StackObj <- data.frame(
  CELL_TYPE = rownames(StackObj), 
  StackObj)
...
# StackObj.long.copy<-StackObj.long

##############################################
#StackObj.long<-StackObj.long.copy
unique(StackObj.long.copy$CELL_TYPE)
StackObj.long$CELL_TYPE2<-gsub("proliferating.", "proliferating", StackObj.long$CELL_TYPE)
StackObj.long$CELL_TYPE2<-gsub("\\."," ", StackObj.long$CELL_TYPE2)
StackObj.long$CELL_TYPE2<-gsub("  "," ", StackObj.long$CELL_TYPE2)

# unique(StackObj.long$SAMPLE_NAME)
# unique(metadata$CELL_TYPE)

# metadata$CELL_TYPE2<-gsub("\\."," ", metadata$CELL_TYPE)
# metadata$CELL_TYPE2<-gsub("  "," ",  metadata$CELL_TYPE2)
# metadata$CELL_TYPE2<-gsub(",","",  metadata$CELL_TYPE2)
# metadata$CELL_TYPE2<-gsub("-","",  metadata$CELL_TYPE2)
# 
...
StackObj.long <- left_join(StackObj.long, celltypes[,
                      c("PATTERN_NAME", "CELL_TYPE_Abb", "CELL_TYPE2")])

# StackObj.long$CELL_TYPE_Abb
# unique(StackObj.long$CELL_TYPE2[c(78:84)])
...
#----------------------------------------------------------------------------
data <-StackObj.long[StackObj.long$NameState%in%c("AconE", "AeloE"),]
data[data$NameState=="AeloE","NameState"]<-"AconE"
data<-data[order(data$NBins,decreasing = TRUE),]
data$CELL_TYPE_Abb<-factor(data$CELL_TYPE_Abb, levels = unique(data$CELL_TYPE_Abb))
data$CELL_TYPE2[45]
# orderdells<-levels(celltypes$CELL_TYPE_Abb)
# orderdells[11:12]<-c("endothelial UmV rest","endothelial UmV prolif")

data4<-aggregate(data3$NBins, by=list(CELL_TYPE2=data3$CELL_TYPE2), FUN=sum)
# colnames(data4)[1]<-"CELL_TYPE2"

data4<-left_join(data4, unique(data3[, c("CELL_TYPE2","MAIN_CELL_TYPE", "CELL_TYPE_Abb")]))
data4$CELL_TYPE_Abb<-as.character(data4$CELL_TYPE_Abb)
data4[12,"CELL_TYPE_Abb"]<-"endothelial UmV prolif"
data4$MAIN_CELL_TYPE<-as.character(data4$MAIN_CELL_TYPE)
data4[12,"MAIN_CELL_TYPE"]<-"endothelial"

data4_ordered<-data4[order(data4$x, decreasing = TRUE),]
data4_ordered$CELL_TYPE_Abb <- 
  factor(data4_ordered$CELL_TYPE_Abb,
  levels = unique(data4_ordered$CELL_TYPE_Abb))
    
data4$CELL_TYPE_Abb <- factor(
  data4$CELL_TYPE_Abb,
  levels =
    c(
      "mesenchymal SC (BM)",
      "adult endothelial progen",
      "endothelial UmV prolif",
      "CD8pos, ab T cell",
      "effec-mem CD8pos, ab T cell",
      "CD4pos, ab T cell",
      "naive B cell",
      "CD38neg naive B cell",
      "germinal center B cell",
      "class switched memory B cell",
      "plasma cell",
      "cytotoxic CD56-dim NK",
      "myelocyte",
      "metamyelocyte",
      "band form neutrophil",
      "segmented neutrophil (BM)",
      "mature neutrophil",
      "mature eosinophil",
      "monocyte",
      "osteoclast",
...
colnames(data4)[c(2,4)]<-c("Conserved_Enhancers", "Cell_Type") 

colnames(data4_ordered)[c(2,4)]<-c("Conserved_Enhancers", "Cell_Type") 

ggplot(data4, aes(y= Conserved_Enhancers, x=Cell_Type)) + 
  geom_bar(stat="identity") + theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 

ggplot(data4_ordered, aes(y= Conserved_Enhancers, x=Cell_Type)) + 
  geom_bar(stat="identity") + theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
